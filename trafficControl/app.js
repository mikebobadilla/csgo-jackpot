...
//
// =================================
queue.process('getStartupInfo', function(job, done){
  resendSocketInfo() 
  done()
})
...
function resendSocketInfo() {
  queue.create('reset', {
    pot: 2, // 1 for regular | 2 for mini
    classids: itemsInPot,
    players: Players,
...
queue.process('updateTradeStatus', function (job, done) {
...
queue.process('addToQueue', function (job, ctx, done) {
...
function checkHand(user, itemsInTrade, result) {
    ...
    if (!user.tradeurl || user.tradeurl == 'NULL' || user.tradeurl == '' || user.tradeurl.length < 8)
      ...
      setTimeout(function () {
        if (totalValue.toFixed(0) < 100 || totalValue.toFixed(0) > 2500) {
...
function cancelTrade(tradeId, done) {
    ...
    var job = queue.create('tradeCancel', {
      tradeid: tradeId
    }).save(function (err) {
...
function acceptTrade(userid, tradeId, items, bot, done) {
  var accept = queue.create('tradeAccept', {
    tradeid: tradeId
  }).save(function (err) {
...
function newRound() {
  ...
  setTimeout(function createNewRound() {
    rounds.create({
      hash: hash,
      salt: salt,
...
setInterval(function () {
  if (ready && !forceOff) {
    var job = queue.create('getJobs', {}).removeOnComplete(true)
      .save(function (err) {
