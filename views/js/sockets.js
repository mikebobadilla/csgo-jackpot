var socket = io('/mini');
    ...
    socket.on('mini_clean', function (data) {
...
//          $("#currentBetsSmall > ul")
    ...
    socket.on('mini_adjustFront', function(data){
    ...
    socket.on('mini_userTickets', function (data) {
      $('.user-percentage > h2').html((data * 100).toFixed(2) + "%")
    })

    socket.on('mini_roundInfo', function (data) {
    ...
    socket.on('mini_winnerInfo', function (data) {
